...
// .wrangler/tmp/bundle-bqBZFx/strip-cf-connecting-ip-header.js
...
var Node = /* @__PURE__ */ __name(class _Node {
  ...
  insert(tokens, index, paramMap, context, isStatic) {
    let node = this;
    for (let i = 0, len2 = tokens.length; i < len2; i++) {
      const token = tokens[i];
      const pattern = token.length === 1 ? token === "*" ? i === len2 - 1 ? ["", "", ONLY_WILDCARD_REG_EXP_STR] : ["", "", LABEL_REG_EXP_STR] : null : token === "/*" ? ["", "", TAIL_WILDCARD_REG_EXP_STR] : token.match(/^\:([^\{\}]+)(?:\{(.+)\})?$/);
      let nextNode;
      if (pattern) {
...
var Node2 = /* @__PURE__ */ __name(class _Node2 {
  ...
  #pushHandlerSets(handlerSets, node, method, nodeParams, params) {
    for (let i = 0, len2 = node.#methods.length; i < len2; i++) {
...
function normalizeSymbol(symbol) {
  ...
  if (s === "NIFTY 50" || s === "NIFTY" || s === "NIFTY_50")
    return "^NSEI";
  if (s === "SENSEX")
    return "^BSESN";
  return s;
...
function getSyntheticData(symbol, period = "1mo") {
  ...
  const basePrice = basePrices[normSym] || 1500;
  const currentPrice = basePrice * (1 + (Math.random() - 0.48) * 0.02);
  ...
  for (let i = 0; i < days; i++) {
    ...
    const vol = Math.floor(Math.random() * 1e6) + 5e5;
    history.push({
  ...
  return {
    ...
    volume: 125e4,
    marketCap: metadata.sector === "Index" ? null : 5e11,
    history,
    isSynthetic: true
  };
}
__name(getSyntheticData, "getSyntheticData");
async function getStockData(symbol, period = "1mo") {
  const normSym = normalizeSymbol(symbol);
  let interval = "1d";
  let range = period;
  if (period === "1d") {
  ...
  try {
    ...
    const quote2 = result.indicators?.quote?.[0] || {};
    const adjClose = result.indicators?.adjclose?.[0]?.adjclose || [];
    const opens = quote2.open || [];
    const highs = quote2.high || [];
    const lows = quote2.low || [];
    const closes = quote2.close || adjClose || [];
    const volumes = quote2.volume || [];
    const history = [];
    for (let i = 0; i < timestamps.length; i++) {
      if (opens[i] === null || opens[i] === void 0 || highs[i] === null || highs[i] === void 0 || lows[i] === null || lows[i] === void 0 || closes[i] === null || closes[i] === void 0) {
        continue;
      }
      const dateStr = new Date(timestamps[i] * 1e3).toISOString().split("T")[0];
      history.push({
        date: dateStr,
    ...
    return {
      ...
      marketCap: null,
      // yahoo chart API doesn't return full market cap easily, set null
      history
    };
  } catch (error) {
...
async function getStockQuotesBatch(symbols) {
  const promises = symbols.map((s) => getStockData(s, "5d"));
...
__name(getStockQuotesBatch, "getStockQuotesBatch");
...
var LLMService = class {
   ...
   */
  async complete(prompt, systemPrompt = "You are a professional financial advisor.", jsonMode = false) {
    const { geminiKey, groqKey, openaiKey } = this.config;
...
User Prompt: ${prompt}` }] }],
   ...
   */
  async analyzeChart(symbol, techData) {
...
Levels: Support: ${techData.levels.support}, Resistance: ${techData.levels.resistance}`;
   ...
   */
  async analyzeStock(symbol, currentPrice, open, high, low, volume) {
...
User Query: ${query}`;
   ...
   */
  async generateDashboardSummary(marketState, watchlist) {
...
${JSON.stringify(holdings, null, 2)}`;
   ...
   */
  generateOfflineFallback(prompt, jsonMode) {
    if (jsonMode) {
...
app.use(
  ...
  cors({
    origin: "*",
    // Adjust to specific Cloudflare Pages URL in production if needed
...
function createSuccessResponse(data, sourceMetadata, confidence, explanation) {
  return {
    success: true,
    data,
    error: null,
    ...
    confidence: confidence !== void 0 ? confidence : null,
...
app.get("/api/v1/history/recent", async (c) => {
  ...
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("analysis_results").select("*").order("created_at", { ascending: false }).limit(limit);
    if (error)
      throw error;
...
app.get("/api/v1/history/daily", async (c) => {
  ...
  try {
    ...
    const { data, error } = await sb.from("analysis_results").select("*").eq("decision", decision).gte("created_at", last24h).order("created_at", { ascending: false });
    if (error)
      throw error;
...
app.get("/api/v1/history/:symbol", async (c) => {
  ...
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("analysis_results").select("*").eq("symbol", symbol).order("created_at", { ascending: false }).limit(limit);
    if (error)
      throw error;
...
app.post("/api/v1/watchlist", async (c) => {
  ...
  if (!symbol)
    return c.json(createErrorResponse("Symbol is required", "VALIDATION_ERROR"), 400);
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("watchlist").upsert({ symbol }, { onConflict: "symbol" }).select();
    if (error)
      throw error;
...
app.post("/api/v1/settings", async (c) => {
  ...
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("user_settings").upsert({ id: "default_user", ...body }, { onConflict: "id" }).select().single();
    if (error)
      throw error;
...
app.get("/api/v1/notifications", async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("notifications").select("*").eq("user_id", "default_user").order("created_at", { ascending: false });
    if (error)
      throw error;
    if (!data || data.length === 0) {
      ...
      const { data: refetched } = await sb.from("notifications").select("*").eq("user_id", "default_user").order("created_at", { ascending: false });
...
app.post("/api/v1/radar/scan", async (c) => {
  ...
  try {
    const quotes = await Promise.all(symbols.map((s) => getStockData(s, "1mo")));
    const llm = getLLM(c.env);
...
app.get("/api/v1/radar/live", async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data } = await sb.from("analysis_results").select("*").order("created_at", { ascending: false }).limit(10);
...
app.get("/api/v1/radar/history", async (c) => {
  ...
  try {
    const sb = getSupabase(c.env);
    const { data } = await sb.from("analysis_results").select("*").order("created_at", { ascending: false }).limit(limit);
    const results = (data || []).map((row) => {
      let parsed = {};
      try {
        parsed = JSON.parse(row.decision_output);
...
app.get("/api/v1/portfolio", async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data: holdings, error } = await sb.from("portfolio_holdings").select("*").eq("user_id", "default_user");
    if (error)
      throw error;
    ...
    const detailedHoldings = (holdings || []).map((h) => {
      ...
      const profitLoss2 = value - cost;
      const profitLossPct2 = cost === 0 ? 0 : profitLoss2 / cost * 100;
      totalInvested += cost;
      totalValue += value;
      return {
        id: h.id,
        ...
        total_cost: cost,
        current_value: value,
        profit_loss: profitLoss2,
        profit_loss_pct: profitLossPct2,
...
app.post("/api/v1/portfolio", async (c) => {
  ...
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("portfolio_holdings").insert({
...
app.put("/api/v1/portfolio/:holding_id", async (c) => {
  ...
  if (body.avg_price !== void 0)
    updates.avg_price = parseFloat(body.avg_price);
  if (body.sector !== void 0)
    updates.sector = body.sector;
  try {
    const sb = getSupabase(c.env);
    const { data, error } = await sb.from("portfolio_holdings").update(updates).eq("id", holdingId).select().single();
    if (error)
      throw error;
    return c.json(createSuccessResponse(data));
  } catch (e) {
...
app.get("/api/v1/portfolio/analysis", async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data: holdings } = await sb.from("portfolio_holdings").select("*").eq("user_id", "default_user");
    const llm = getLLM(c.env);
...
app.get("/api/v1/market/news/story-arcs", (c) => {
  return c.json(
    createSuccessResponse([
      { arc_name: "Nifty Bull Run", stories: 15, last_updated: (/* @__PURE__ */ new Date()).toISOString() },
      { arc_name: "Sensex Consolidation", stories: 8, last_updated: (/* @__PURE__ */ new Date()).toISOString() }
    ])
  );
});
...
app.post("/api/v1/chat", async (c) => {
  ...
  if (!query)
    return c.json(createErrorResponse("Query is required", "VALIDATION_ERROR"), 400);
  try {
    const llm = getLLM(c.env);
...
// .wrangler/tmp/bundle-bqBZFx/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bqBZFx/middleware-loader.entry.ts
